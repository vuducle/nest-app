...
import { Playlist, SpotifyTrack, playlistsApi, spotifyApi } from "@/lib/api";
import {
  Music,
  Search,
  Plus,
...
> = ({ isOpen, onClose, playlist, onSongsAdded }) => {
  ...
  const handleAddSongsToPlaylist = async () => {
    ...
    try {
      for (const track of selectedSongs) {
        await playlistsApi.addSpotifyTrackToPlaylist(playlist.id, track.id);
      }
      onSongsAdded();
      onClose();
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Search for K-pop songs and add them to your playlist
          </DialogDescription>
        </DialogHeader>
