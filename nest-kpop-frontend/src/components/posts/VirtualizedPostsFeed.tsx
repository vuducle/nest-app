...
interface VirtualizedPostsFeedProps {
  userId?: string;
...
const PostItem: React.FC<PostItemProps> = ({ index, style, data }) => {
  const { posts, onLike, onComment } = data;
  ...
  return (
    <div style={style} className="px-4">
      <div className="mb-6">
        <PostCard post={post} onLike={onLike} onComment={onComment} />
      </div>
    </div>
  );
};

export const VirtualizedPostsFeed: React.FC<VirtualizedPostsFeedProps> = ({
  userId,
